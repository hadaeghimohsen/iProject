...
CREATE VIEW [Sas].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      Sas.Privilege.ShortCut AS PrivilegeID, Sas.Privilege.TitleEn AS PrivilegeEnName, Sas.Privilege.IsVisible AS PrivilegeVisible, 
                      Sas.Privilege.TitleFa AS PrivilegeFaName
FROM         DataGuard.Role INNER JOIN
             DataGuard.Role_Privilege ON DataGuard.Role.ID = DataGuard.Role_Privilege.RoleID INNER JOIN
             Sas.Privilege ON DataGuard.Role_Privilege.PrivilegeID = Sas.Privilege.ID INNER JOIN
...
WHERE     (Sas.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
