...
CREATE VIEW [Gas].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      [Gas].Privilege.ShortCut AS PrivilegeID, [Gas].Privilege.TitleEn AS PrivilegeEnName, [Gas].Privilege.IsVisible AS PrivilegeVisible, 
                      [Gas].Privilege.TitleFa AS PrivilegeFaName
FROM         DataGuard.Role INNER JOIN
                      DataGuard.Role_Privilege ON DataGuard.Role.ID = DataGuard.Role_Privilege.RoleID INNER JOIN
                      [Gas].Privilege ON DataGuard.Role_Privilege.PrivilegeID = [Gas].Privilege.ID INNER JOIN
...
WHERE     ([Gas].Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
