...
CREATE VIEW [ServiceDef].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      ServiceDef.Privilege.ShortCut AS PrivilegeID, ServiceDef.Privilege.TitleEn AS PrivilegeEnName, ServiceDef.Privilege.IsVisible AS PrivilegeVisible, 
...
FROM         DataGuard.Role INNER JOIN
                      ...
                      ServiceDef.Privilege ON DataGuard.Role_Privilege.PrivilegeID = ServiceDef.Privilege.ID INNER JOIN
...
WHERE     (ServiceDef.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
