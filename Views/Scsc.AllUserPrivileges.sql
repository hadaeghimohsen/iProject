...
CREATE VIEW [Scsc].[AllUserPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].IsVisible AS UserVisible, DataGuard.[User].IsLock AS UserLock, DataGuard.[User].TitleEn AS UserTitle, 
                      ...
                      Scsc.Privilege.IsVisible AS PrivilegeVisible, Scsc.Privilege.ID AS PrivilegeID, Scsc.Privilege.ShortCut, 
...
WHERE     (DataGuard.[User].IsVisible = 1) AND (DataGuard.User_Privilege.IsVisible = 1) AND (DataGuard.[User].IsLock = 0) AND (Scsc.Privilege.IsVisible = 1)
