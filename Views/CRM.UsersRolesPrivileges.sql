...
CREATE VIEW [CRM].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      CRM.Privilege.ShortCut AS PrivilegeID, CRM.Privilege.TitleEn AS PrivilegeEnName, CRM.Privilege.IsVisible AS PrivilegeVisible, 
...
FROM         DataGuard.Role INNER JOIN
             ...
             CRM.Privilege ON DataGuard.Role_Privilege.PrivilegeID = CRM.Privilege.ID INNER JOIN
...
WHERE     (CRM.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
