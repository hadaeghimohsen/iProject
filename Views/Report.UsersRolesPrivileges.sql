...
CREATE VIEW [Report].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      Report.Privilege.ShortCut AS PrivilegeID, Report.Privilege.TitleEn AS PrivilegeEnName, Report.Privilege.IsVisible AS PrivilegeVisible, 
...
FROM         DataGuard.Role INNER JOIN
                      ...
                      Report.Privilege ON DataGuard.Role_Privilege.PrivilegeID = Report.Privilege.ID INNER JOIN
...
WHERE     (Report.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
