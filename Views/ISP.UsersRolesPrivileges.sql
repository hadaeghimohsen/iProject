...
CREATE VIEW [ISP].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      ISP.Privilege.ShortCut AS PrivilegeID, ISP.Privilege.TitleEn AS PrivilegeEnName, ISP.Privilege.IsVisible AS PrivilegeVisible, 
...
FROM         DataGuard.Role INNER JOIN
             ...
             ISP.Privilege ON DataGuard.Role_Privilege.PrivilegeID = ISP.Privilege.ID INNER JOIN
...
WHERE     (ISP.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
