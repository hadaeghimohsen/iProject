...
CREATE VIEW [Msgb].[UsersRolesPrivileges]
AS
SELECT     DataGuard.[User].ID AS UserID, DataGuard.[User].TitleFa AS UserFaName, DataGuard.[User].TitleEn AS UserEnName, DataGuard.[User].IsLock AS UserLock, 
                      ...
                      Msgb.Privilege.ShortCut AS PrivilegeID, Msgb.Privilege.TitleEn AS PrivilegeEnName, Msgb.Privilege.IsVisible AS PrivilegeVisible, 
...
FROM         DataGuard.Role INNER JOIN
             ...
             Msgb.Privilege ON DataGuard.Role_Privilege.PrivilegeID = Msgb.Privilege.ID INNER JOIN
...
WHERE     (Msgb.Privilege.IsVisible = 1) AND (DataGuard.Role_Privilege.IsActive = 1) AND (DataGuard.Role_Privilege.IsVisible = 1) AND (DataGuard.Role.IsVisible = 1) AND 
