...
-- =============================================
CREATE PROCEDURE [DataGuard].[GRRole]
...
BEGIN
	...
	SELECT @Bpid = @x.query('.').value('(BoxPrivilege/@bpid)[1]', 'BIGINT'),
	       @Roleid = @x.query('.').value('(BoxPrivilege/@roleid)[1]', 'BIGINT'),
	...
	BEGIN
      ...
      ELSE IF @SubSys = 5
	   ...
	   EXEC Global.GrantPrivilegesToRole @Xml = @XP -- xml	   	   
	END
	...
	BEGIN
      ...
      ELSE IF @SubSys = 5
	   ...
	   EXEC Global.RevokePrivilegesFromRole @Xml = @XP -- xml
	END	
END
GO
